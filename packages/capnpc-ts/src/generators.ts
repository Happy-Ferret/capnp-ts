...
export function generateDefaultValue(node: s.Node, field: s.Field, fieldIndex: number): ts.PropertyAssignment {
  ...
  switch (whichSlotType) {
    ...
    case s.Type_Which.TEXT:
      ...
      initializer = ts.createCall(
        ts.createPropertyAccess(CAPNP, 'getTextDefault'), __,
        [ts.createLiteral(nodeIdHex), ts.createLiteral(fieldIndex)]);
...
export function generateFileRequest(ctx: CodeGeneratorFileContext) {

  trace('generateFileRequest()');

  const messageLiteral = ts.createArrayLiteral(capnp.dumpSchema(ctx.req).map(ts.createLiteral), true);

  const codeGenStruct = ctx.nodes.some(
    (n) => n.getDisplayName().substr(n.getDisplayNamePrefixLength()) === 'CodeGeneratorRequest')
    // HACK: If a CodeGeneratorStruct exists in the file, use that struct instead. This avoids a circular import in
    // schema.capnp.ts.
    ? ts.createIdentifier('CodeGeneratorRequest')
    : ts.createPropertyAccess(CAPNP, 'CodeGeneratorRequest');

  // capnp.registerSchema(capnp.CodeGeneratorRequest, _capnpFileId, []);
  ctx.statements.push(
    ts.createStatement(
      ts.createCall(
        ts.createPropertyAccess(CAPNP, 'registerSchema'), __, [
          codeGenStruct,
          ts.createIdentifier('_capnpFileId'),
          messageLiteral,
        ])));
...
export function generateStructNode(ctx: CodeGeneratorFileContext, node: s.Node, interfaceNode: boolean): void {
  ...
  members.push(...consts.map((n) => createConstProperty(node, n)));
  ...
  const defaultValues = fields.reduce(
    (acc, f, i) => f.isSlot() && f.getSlot().getHadExplicitDefault() && f.getSlot().getType().which() !== s.Type.VOID
      ? acc.concat(generateDefaultValue(node, f, i))
      : acc,
